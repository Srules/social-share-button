window.SocialShareButton =
  ...
  share : (el) ->
    ...
    sub = $parent.getAttribute("data-sub") || 'https://reddit.com'
    ...
    switch site
      ...
      when "weibo"
        SocialShareButton.openUrl("https://service.weibo.com/share/share.php?url=#{url}&type=3&pic=#{img}&title=#{title}&appkey=#{appkey}", 620, 370)
      when "twitter"
      ...
      when "douban"
        SocialShareButton.openUrl("https://shuo.douban.com/!service/share?href=#{url}&name=#{title}&image=#{img}&sel=#{desc}", 770, 470)
      when "facebook"
        SocialShareButton.openUrl("https://www.facebook.com/sharer/sharer.php?u=#{url}&display=popup&quote=#{desc}", 555, 400)
      when "qq"
        SocialShareButton.openUrl("https://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url=#{url}&title=#{title}&pics=#{img}&summary=#{desc}&site=#{appkey}")
      when "google_plus"
      ...
      when "vkontakte"
        SocialShareButton.openUrl("https://vk.com/share.php?url=#{url}&title=#{title}&image=#{img}")
      when "odnoklassniki"
      ...
      when "tumblr"
        ...
        SocialShareButton.openUrl("https://www.tumblr.com/share#{tumblr_params()}")

      when "reddit"
        SocialShareButton.openUrl("#{sub}/submit?url=#{url}&title=#{desc}&newwindow=1", 555, 600)
      when "hacker_news"
        SocialShareButton.openUrl("https://news.ycombinator.com/submitlink?u=#{url}&t=#{title}", 770, 500)
      when "telegram"
